...
export default function Attendance() {
  const { restaurant, totalCovers, lunchCovers, dinnerCovers } = useAppData();
  ...
  return <div className="page attendance-page">
    ...
    <div className="stats-grid four"><StatCard icon={Users} value={totalCovers} label="Coperti prenotati" /><StatCard icon={Armchair} value={Math.max(0, totalCapacity - totalCovers)} label="Posti disponibili" /><StatCard icon={TrendingUp} value={`${Math.round((totalCovers / totalCapacity) * 100)}%`} label="Occupazione" tone="green" /><StatCard icon={Clock3} value="2,3" label="Turnover medio" /></div>
    <div className="attendance-grid">
      ...
      <Panel className="service-donuts"><SectionTitle>Coperti per servizio</SectionTitle><div className="donut-row"><Donut value={lunchCovers} max={lunchCapacity} label="Pranzo" color="#43bf7e" /><Donut value={dinnerCovers} max={dinnerCapacity} label="Cena" color="#ef5a54" /></div></Panel>
    </div>

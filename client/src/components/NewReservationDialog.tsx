...
export default function NewReservationDialog() {
  const { modalOpen, setModalOpen, addReservation, reservations } = useAppData();
  ...
  const selectedDate = day === "Oggi" ? TODAY : day === "Domani" ? "2026-09-17" : "2026-09-18";
  const availability = useMemo(() => {
    const occupied = reservations.filter((item) => item.date === selectedDate && item.time === time && item.status !== "cancelled").reduce((sum, item) => sum + item.guests, 0);
    return Math.max(0, 24 - occupied);
  ...
  return (
    <div className="dialog-backdrop" onMouseDown={close}>
      <form className="reservation-dialog" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="dialog-grid">
          <div className="dialog-form">
            ...
            <div className="field"><span>Data</span><Segmented options={["Oggi", "Domani", "Scegli data"]} value={day} onChange={setDay} /></div>
          ...
          <aside className="availability-panel">
            ...
            <div className="availability-score"><span><Check /></span><div><strong>{availability} coperti</strong><small>disponibili alle {time}</small></div></div>
            <div className="availability-meta"><CalendarDays size={18} /><span>{day}, {service.toLowerCase()}</span></div>
        ...
        <PrimaryButton type="submit" className="dialog-submit">Conferma prenotazione</PrimaryButton>
      </form>
    </div>
  );
}
